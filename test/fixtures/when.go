...
func (w *When) WaitForMotoSvcReady(motoSVCName string) *When {
	w.t.Helper()
	ctx := context.Background()
	if err := WaitForMotoSvcStatefulSetReady(ctx, w.kubeClient, Namespace, motoSVCName, 5*time.Minute); err != nil {
		w.t.Fatal(err)
	}
	return w
}
